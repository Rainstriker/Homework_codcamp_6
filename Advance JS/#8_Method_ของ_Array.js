...
let array2 = array1.map(x => Object.assign(
  x, {
      age: Number( x.birth.substr(0, 4) )
      }
  )
);
...
let array2 = array1.map(x => `<tr> <td>${x.name}</td> <td>${x.birth}</td></tr>`);
...
let array2 = array1.filter(x => Number( x.birth.substr(0, 4) ) < 2000);
